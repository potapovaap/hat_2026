...
protocol addOrEditPlayersDelegate{
    func closeAddOrEditPlayerVC()
}

class addOrEditPlayerVC : UIViewController,
                UITextFieldDelegate
{
    ...
    var delegate : addOrEditPlayersDelegate?
    var moc: NSManagedObjectContext?
    var isEditPlayerMode = false
    var players: [Player] = []
    var player: Player?
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        navigationItem.title = lang_scheme[cur_lang]?["addOrEditPlayerVCTitle"] as? String ?? "navigationBar title"
    ...
    func updateInterface() {
        if isEditPlayerMode {
            ...
            navigationItem.title = lang_scheme[cur_lang]?["alertEditPlayerTitle"] as? String ?? "navigationBar title"
            ...
            cancelButton.setTitle(cancelTitle,for: .normal)
            navigationItem.title = lang_scheme[cur_lang]?["alertAddPlayerTitle"] as? String ?? "navigationBar title"
    ...
    func okAction() {
        ...
        if isEditPlayerMode {  editPlayer() }
        else { addPlayer() }
    }
    ...
    func addPlayer() {
        ...
        // добавляем в own
        var ownPlayersIds = (userDefaults.object(forKey: "ownPlayersIds") as? [Int]) ?? []
        ownPlayersIds.append(Int(newPlayer.id))
        userDefaults.set(ownPlayersIds, forKey: "ownPlayersIds")
    ...
    func checkData() -> Bool {
        ...
        for i in 0..<textfieldsArray.count {
            ...
            let oldFieldValue = player?.value(forKey: fieldsArray[i]) as? String ?? ""
            let allPlayersFieldValue = players.map { ($0.value(forKey: fieldsArray[i]) as? String)?.lowercased() ?? ""}
            fieldIsCorrect = checkField(field: fieldsArray[i], newFieldValue: newFieldValue, oldFieldValue: oldFieldValue, allPlayersFieldValue: allPlayersFieldValue, fieldMinimumSymbolsCount: minimumSymbolsCountArray[i], fieldMaximumSymbolsCount: maximumSymbolsCountArray[i])
            if !fieldIsCorrect { return false }
        }
        return true
    }
    
    func checkField(field: String, newFieldValue: String, oldFieldValue: String, allPlayersFieldValue: [String], fieldMinimumSymbolsCount: Int, fieldMaximumSymbolsCount: Int) -> Bool {
        ...
        if newFieldValue.lowercased() != oldFieldValue.lowercased() && allPlayersFieldValue.contains(newFieldValue.lowercased()) {
    ...
    func closeController(){
        delegate?.closeAddOrEditPlayerVC()
        goBackVC()
    }
